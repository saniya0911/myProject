...
root = Tk()
root.title("Saniya's Notepad")
root.geometry("800x500")
root.wm_iconbitmap("my_image.ico")
...
def open_file():
    ...
    if file is not None:
        ...
        file.close()
        root.title(f"Saniya's Notepad - {file.name}")
...
def save_file():
    ...
    if file is not None:
        ...
        file.close()
        root.title(f"Saniya's Notepad - {file.name}")
        messagebox.showinfo("Info", "File has been saved successfully!")
